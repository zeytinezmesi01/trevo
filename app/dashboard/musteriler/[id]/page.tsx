            ...
            <tbody>
              {invoices.map(inv => (
                <tr key={inv.id as string} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}
                  ...
                  <td style={{ padding: '10px 20px' }}><span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 100, background: inv.status === 'paid' ? '#ecfdf5' : '#eef2ff', color: inv.status === 'paid' ? '#10b981' : 'var(--brand-primary, #4f7dff)' }}>{inv.status === 'paid' ? 'Ödendi' : inv.status === 'sent' ? 'Gönderildi' : 'Taslak'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
